...
### P7 — Sichtbarkeits-LoS pro Spieler cachen (skaliert mit Spielerzahl) — ✅ umgesetzt (2026-07-05, Branch `perf/p7-los-cache`; aus der Schublade geholt auf User-Entscheid: TTL 1,5s statt 0,1s (`PLAYER_LOS_TTL_S`), Cache nur im `_should_update_player`-Pfad via `now`-Parameter von `_sees_in_window` — Targeting-Aufrufer rechnen weiter exakt; Staleness wirkt praktisch nur auf ST-Träger, radar-sichtbare Gegner laufen über den Radar-Pfad weiter. Tests: TestPlayerLosCache in test_targeting.py)
...
| P7 ✅ | Sichtbarkeits-LoS-TTL pro Spieler (TTL 1,5s) | ⭐⭐ (viele Spieler) | ○ | niedrig | klein |
...
### N2 — Leerlauf-Early-Outs (Nachmessung tmp4: ~2–2,5% der aktiven CPU) — ✅ umgesetzt (Branch `perf/idle-early-outs`, Commit `b764a30`; Early-Outs in `_resolve_incoming_shots`/`_cleanup_shots`/`_find_incoming_shot`, tests/test_idle_early_out.py)
...
### N3 — Tick-Wait verschlanken (Gate erfüllt) — ✅ umgesetzt (Branch `perf/idle-early-outs`, Commit `fbbe86d`; Entscheid: `time.sleep`, stop-Latenz ≤16ms)

`Event.wait(timeout)` pro Tick kostet ~0,4%/Bot (Condition-Objekt + Lock-Maschinerie). `time.sleep` wäre billiger, verzögert aber `stop()` um bis zu 16ms. Das ursprüngliche Gate („nur falls Nachmessung >10%") ist erfüllt (tmp4: 15–17% docker). Nachmessung tmp4: Event.wait-Maschinerie ≈7% der aktiven CPU PLUS ~230 Futex-Wakeups/s, deren Kernel-Anteil cProfile nicht sieht — der Punkt wirkt also doppelt. Umsetzungsentscheid: `time.sleep` (einzelner Syscall statt Condition/Lock-Maschinerie); `stop()` greift über `_running`/`_stop_event` am nächsten Schleifenkopf — Latenz maximal eine Tick-Dauer (~16ms), akzeptiert. Der einmalige 0,5s-Pre-Spawn-Wait bleibt auf `Event.wait` (unterbrechbar, nicht heiß).
...
**Nachmessung nach N2+N3 (2026-07-05): ✅ regelmäßig <15% docker.** Das Ziel <10–12% wurde nicht ganz erreicht, die Verbesserung gegenüber 15–17% ist aber bestätigt — der Rest ist der oben beschriebene Prozess-Floor (Kernel-/Syscall-Anteil ×4 Interpreter), den nur Track 5 adressiert.
